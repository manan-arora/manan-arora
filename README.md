<h1 align="center">Hi 👋, I'm Manan Arora</h1>

<h3 align="center">Full Stack Developer | Building Scalable Web Apps with TypeScript, Next.js & Node.js</h3>

<p align="center">
I build production-ready full-stack applications with modern web technologies,
secure authentication, scalable backend systems, and polished user experiences.
</p>

---

## 🚀 Current Focus

- ⚡ Building full-stack apps using **Next.js + TypeScript**
- 🤖 Exploring **AI-powered product development**
- 🧠 Practicing **Data Structures & Algorithms**
- 🌱 Learning **System Design & PostgreSQL**

---

## ⭐ Featured Projects

### Unsaid - AI-Assisted Anonymous Messaging Platform

A modern anonymous messaging platform where users get a public inbox link to receive anonymous messages with AI-assisted message suggestions.

#### Features
- 🔐 Auth.js Authentication
- 📧 OTP Email Verification
- 📨 Anonymous Messaging
- 🔗 Public Profile Links
- 🤖 Gemini AI Message Suggestions
- 🎛 Inbox Toggle (Accept / Pause Messages)
- 🎨 Custom Neo-Brutalist UI

**Tech Stack:**  
Next.js • TypeScript • MongoDB • Mongoose • Tailwind CSS • Auth.js • Gemini API

**🔗 Live Demo:**  
https://try-unsaid.vercel.app

**🔗 Repository:**  
https://github.com/manan-arora/unsaid-anonymous-messaging

---

### Workstream - Production-Ready Project Management Backend

Production-grade backend architecture for project management workflows.

#### Features
- 🔐 JWT Authentication
- 🔄 Refresh Token Rotation
- 👥 Role-Based Access Control (RBAC)
- 📧 Email Verification & Password Reset
- 📁 File Attachments
- 📝 Project Notes
- 📋 Task & Subtask Workflows
- 📖 Swagger Documentation
- 🚀 Production Deployment

**Tech Stack:**  
Node.js • Express.js • MongoDB • JWT • Swagger

**🔗 Live API Docs:**  
https://workstream-06tn.onrender.com/api/v1/docs

**🔗 Repository:**  
https://github.com/manan-arora/project-management-backend

---

## 💻 Tech Stack

### Languages
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)

### Frontend
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind-06B6D4?style=for-the-badge&logo=tailwindcss&logoColor=white)

### Backend
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-000000?style=for-the-badge&logo=express&logoColor=white)
![Auth.js](https://img.shields.io/badge/Auth.js-000000?style=for-the-badge)

### Database / ORM
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white)
![Prisma](https://img.shields.io/badge/Prisma-2D3748?style=for-the-badge&logo=prisma&logoColor=white)

### AI / Tools
![Gemini](https://img.shields.io/badge/Gemini_AI-4285F4?style=for-the-badge)
![JWT](https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=jsonwebtokens&logoColor=white)
![Swagger](https://img.shields.io/badge/Swagger-85EA2D?style=for-the-badge&logo=swagger&logoColor=black)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)

---

## 📊 GitHub Stats

<table align="center"> 
  <tr>
    <td> 
      <img src="https://streak-stats.demolab.com?user=manan-arora&theme=github-dark-blue&date_format=j%20M%5B%20Y%5D" /> </td> 
    <td> 
      <img src="https://github-stats-extended.vercel.app/api?username=manan-arora&rank_icon=github&show_icons=true&include_all_commits=true&theme=github_dark" /> 
    </td> 
  </tr> 
</table>

<img src="https://github-readme-activity-graph.vercel.app/graph?username=manan-arora&theme=github-dark&hide_border=true" />



---

## 🌐 Connect With Me

- LinkedIn: https://www.linkedin.com/in/mananarora04
- LeetCode: https://leetcode.com/u/mananarora04
- Email: **mananarora1350@gmail.com**


